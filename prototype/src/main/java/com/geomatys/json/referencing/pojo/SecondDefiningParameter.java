...
public class SecondDefiningParameter {
    ...
    public SecondDefiningParameter(org.opengis.referencing.datum.Ellipsoid ellipsoid) {
        isSphere = ellipsoid.isSphere();
        if (ellipsoid.isIvfDefinitive()) {
            inverseFlattening = new Measure(ellipsoid.getInverseFlattening());
        } else {
            semiMinorAxis = new Measure(ellipsoid.getSemiMinorAxis(), ellipsoid.getAxisUnit());
        }
    }
}
